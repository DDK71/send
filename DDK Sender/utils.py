"""
Utilitários, Logger e Interface Visual — DDK v3.0
"""
...
# ── IP Cache (evita requisições HTTP repetidas) ────────
_cached_ip = None
_cached_ip_lock = threading.Lock()


def get_current_ip():
    """Obtém IP público uma única vez e cacheia para toda a sessão"""
    global _cached_ip
...
class Display:
    ...
    def banner():
        console.clear()
        title = Text()
        title.append("⚡ DDK MASS MAILER v3.0 ⚡\n", style="bold cyan")
        title.append(
            "Multi-Thread • Multi-SMTP • Spintax • Hash Buster • Inbox Engine",
            style="dim white",
        )
        panel = Panel(
            Align.center(title),
    ...
    def show_inputs_overview(smtp_count, proxy_count, emails_count, subjects_count):
        ...
        table.add_row(
            "Proxies",
            f"{proxy_count}" if proxy_count > 0 else "[yellow]0 (direto)[/yellow]",
        )
    ...
    def show_campaign_summary(settings, smtp_count, total_emails):
        table = Table(
            title="⚙️ Configuração da Campanha",
            box=box.HEAVY_EDGE,
        ...
        table.add_row("Threads", str(settings["num_threads"]))
        table.add_row("Delay (base)", f"{settings['delay']}s + jitter")
        table.add_row(
            "Proxy",
            "[green]Ativo[/green]" if settings["use_proxy"] else "[dim]Off[/dim]",
        )
        table.add_row(
            "Spintax",
            "[green]Ativo[/green]" if settings.get("spintax") else "[dim]Off[/dim]",
        )
        table.add_row(
            "Hash Buster",
            "[green]Ativo[/green]"
            if settings.get("hash_buster")
            else "[dim]Off[/dim]",
        )
        table.add_row(
            "Warmup",
            "[green]Ativo[/green]" if settings.get("warmup") else "[dim]Off[/dim]",
        )
        table.add_row(
            "Anexos",
            "[green]Sim[/green]"
            if settings.get("use_attachments")
            else "[dim]Não[/dim]",
        )
    ...
    @staticmethod
    def success(smtp_str, recipient, ip_info=None):
        with Display._lock:
            Display.sent += 1
            ip = ip_info or ""
            console.print(
    ...
    def warning(msg):
        console.print(
            f"  [bold yellow]⚠[/bold yellow] [yellow]{msg}[/yellow]", highlight=False
        )
...
def load_file_lines(filepath):
    """Carrega linhas de arquivo com fallback de encoding"""
...
def generate_message_id(domain):
    """Gera Message-ID RFC-compliant único"""
